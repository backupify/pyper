...
module Pyper::Pipes::Cassandra
  class PagingReaderTest < Minitest::Should::TestCase
    context 'a cassandra paging_reader pipe' do
      setup do
        setup_cass_schema

        @client = create_cass_client('test_datastore')
        @pipe = PagingReader.new(:test, @client, mod_size = 4)
      ...
      should 'page through cassandra result and return all items' do
        ...
        paging_pipe = PagingReader.new(:test, @client, page_size = 1)
        result = paging_pipe.pipe(id: 'ida').to_a
